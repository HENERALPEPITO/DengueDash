...
\begin{table}[h!]
	...
	\begin{tabular}{|l|c|c|c|}
		...
		\textbf{LSTM} & \textbf{260.93} & \textbf{16.15} & \textbf{9.30} \\ \hline
...
The evaluation metrics included Mean Squared Error (MSE), Root Mean Squared Error (RMSE), Mean Absolute Error (MAE) and R² Score, which assess the accuracy of the model's predictions.
\begin{table}[h!]
	\centering
...
Using the 10-week sequence length identified as the optimal window size in preliminary experiments, the dataset was reshaped accordingly and served as the input for hyperparameter tuning. The tuning process was conducted using the \textit{Bayesian Optimization} approach provided by the \texttt{Keras Tuner} library, targeting the minimization of validation loss (Mean Squared Error). The key hyperparameters explored during the tuning were:

\begin{itemize}
	\item \textbf{LSTM units:} 256
...
These results emphasize the importance of critically evaluating tuning results and underscore that automated hyperparameter optimization does not always guarantee better model performance on unseen data.
